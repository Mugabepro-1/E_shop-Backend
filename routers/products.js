...
router.post(
  ...
  async (req, res) => {
    ...
    const file = req.file;
    if (!file)
      return res.status(400).send("There is no image file in this request");
  ...
  const product = Product.findByIdAndDelete(req.params.id)
    .then((product) => {
      if (product)
        ...
        return res
          .status(404)
          .json({ success: false, message: "The product was not deleted" });
      }
    })
    .catch((err) => {
...
router.put(
 ...
 async (req, res) => {
    if (!mongoose.isValidObjectId(req.params.id)) {
      return res.status(400).send("Invalid produt");
    }
    try {
      ...
      if (!product) {
        return res.status(400).send("Failded to upload the product");
      }
      res.json(product);
    } catch (error) {
      console.log(error);
